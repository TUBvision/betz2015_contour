...
%
function [ind, wp] = CIWaM(img, window_sizes, wlev, gamma, srgb_flag, nu_0, adaptor, alpha, mu, sigma)
...
% extended by Torsten Betz to handle temporal adaptation

% compute responses to adaptor if present
if nargin < 7
    adaptor = nan;
    alpha = nan;
    mu = nan;
...
if ndims(img) == 2
    ...
    [ind, wp] = CIWAM_per_channel(img, wlev, nu_0, 'intensity', window_sizes, adaptor, alpha, mu, sigma);
else
    opp_img = rgb2opponent(img, gamma, srgb_flag);
    
    % generate ciwam for each channel:
    rec(:,:,1) = CIWAM_per_channel(opp_img(:,:,1),wlev,nu_0,'colour',window_sizes, adaptor, alpha, mu, sigma);
    rec(:,:,2) = CIWAM_per_channel(opp_img(:,:,2),wlev,nu_0,'colour',window_sizes, adaptor, alpha, mu, sigma);
    rec(:,:,3) = CIWAM_per_channel(opp_img(:,:,3),wlev,nu_0,'intensity',window_sizes, adaptor, alpha, mu, sigma);
...
function [rec, wp] = CIWAM_per_channel(channel,wlev,nu_0,mode,window_sizes, adaptor, adapt_alpha, mu, sigma)
...
if ~any(isnan(adaptor))
    adaptor_response = DWT(adaptor, wlev); 
end
...
for s = 1:wlev
    ...
    for orientation = 1:3
        ...
        % apply adaptation
        if ~any(isnan(adaptor))
            response_adapt = abs(adaptor_response{s,1}(:,:,orientation));
            adapt_weights = (1 - (1 - adapt_alpha) .* normcdf(response_adapt, mu, sigma));
            ws = ws .* adapt_weights;
        end
