...
function [modelOut, normalizers] = flodog_normalize(filter_response, sigx, sr,  sdmix, PIXELS_PER_DEGREE)

if nargin < 5
    PIXELS_PER_DEGREE = 40.8; %#ok<NASGU> the value is used inside BM_model_params
end
add_const = 10^(-6);  
...
BM_model_params
normalizers = cell(length(orientations), length(stdev_pixels));
...
for o = 1 : length(orientations)
    ...
    for f = 1 : length(stdev_pixels)
        ...
        normalizers{o, f} = local_normalizer;
